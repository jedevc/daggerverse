...
func (r Dockerc) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Version string
	}
...
func (r *Dockerc) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Version string
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Dockerc":
		...
		case "Compile":
			...
			return (*Dockerc).Compile(&parent, ctx, ctr)
		case "":
			var parent Dockerc
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Dockerc").
					WithFunction(
					...
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Dockerc")).
							WithArg("version", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"latest\"")}))), nil
	default:
